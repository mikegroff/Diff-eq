% Search for awesome case of double pendulum problem
clear; close all;
m1 = 2; m2 = 6; l1 = 4; l2 = 3; g = 9.81;
f1 = @(x) m2*l2*x(4)^2*sin(x(1)-x(2)) - (m1+m2)*g*sin(x(1));
f2 = @(x) -l1*x(3)^2*sin(x(1)-x(2)) - g*sin(x(2));
F = @ (t,x) [x(3); x(4);
    ( l2*f1(x) - m2*l2*cos(x(2)-x(1))*f2(x) )/( (m1+m2)*l1*l2 - m2*l1*l2*cos(x(2)-x(1))^2 );
    ( -l1*cos(x(2)-x(1))*f1(x) + (m1+m2)*l1*f2(x) )/( (m1+m2)*l1*l2 - m2*l1*l2*cos(x(2)-x(1))^2 )];
%
x(1) = 2; x(2) = 3*pi; x(3) = 2; x(4) = 4; h = 0.01;
t = 0; X = zeros(4,1/h + 1);
X(:,1) = [x(1);x(2);x(3);x(4)];
figure(1);
plot([0 l1*sin(X(1,1))],[0, - l1*cos(X(1,1))],'-o',...
    [l1*sin(X(1,1)), l1*sin(X(1,1)) + l2*sin(X(2,1))],[-l1*cos(X(1,1)), -l1*cos(X(1,1)) - l2*cos(X(2,1))],'-o',...
...
for k=1:10/h
    ...
    t(k+1) = t(k) + h;
    plot([0 l1*sin(X(1,k+1))],[0, - l1*cos(X(1,k+1))],'-o',...
        [l1*sin(X(1,k+1)), l1*sin(X(1,k+1)) + l2*sin(X(2,k+1))],[- l1*cos(X(1,k+1)),- l1*cos(X(1,k+1)) - l2*cos(X(2,k+1))],'-o',...
        'MarkerSize',5,'LineWidth',3);
    ...
    title(sprintf('Evolution at t = %g',t(k+1))); pause(0.001 )
end    
